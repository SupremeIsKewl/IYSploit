if IY_LOADED and not _G.IY_DEBUG == true then
	error("IYSploit is already running!",0)
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'IYSploit is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Credits.Text = "IYSploit(70+ Commands) By RSF#4715"
...
About.TextSize = 14
About.Text = "Plugins are .iys files and should be located in the 'workspace' folder of your exploit."
...
PluginsHint.Text = "Download plugins from the IYS Discord (dsc.gg/iysploit)"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		...
		inviteButton.Text = "dsc.gg/iysploit"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
function saves()
	if writefileExploit() then
		if pcall(function() readfile("IYS_FE.iys") end) then
			if readfile("IYS_FE.iys") ~= nil then
				local success, response = pcall(function()
					local json = game:GetService("HttpService"):JSONDecode(readfile("IYS_FE.iys"))
					if json.prefix ~= nil then prefix = json.prefix else prefix = 'z' end
				...
				if not success then
					...
					writefileCooldown("IYS_FE.iys", defaults)
					wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("IYS_FE.iys", game:GetService("HttpService"):JSONEncode(update))
	end
end
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- IYSploit Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs[#CMDs + 1] = {NAME = 'gamepass', DESC = 'Gets A Gamepass'}
CMDs[#CMDs + 1] = {NAME = 'scripthub / shub', DESC = 'Loads IYSHub(50+ Scripts)'}
...
CMDs[#CMDs + 1] = {NAME = 'hideiys', DESC = 'Hides the main IYS GUI'}
CMDs[#CMDs + 1] = {NAME = 'showiys', DESC = 'Shows IYS again'}
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'iys_fe.iys' or name == 'iys_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.iys' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.iys') end)
			fileName = name..'.iys'
		end
		if file then
...
addcmd('hideiys',{},function(args, speaker)
	...
	minimizeNum = 0
	minimizeHolder()
	notify('IYS Hidden','You can press the prefix key to access the command bar')
end)

addcmd('showiys',{},function(args, speaker)
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
		StayOpen = true
...
addcmd('scripthub',{'shub'},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Ruben162/IYSploit/main/Loader_SH.lua", true))()
        notify('IYScriptHub','IYScriptHub (x)')
end)
...
spawn(function()
	if pcall(function() loadstring(game:HttpGet('https://raw.githubusercontent.com/Ruben162/IYSploit/main/FE_version.txt'))() end) then
		if ver ~= Version then
			notify('Outdated','Get the new version at raw.githubusercontent.com/Ruben162/IYSploit/main/Loader_FE.lua')
		end
