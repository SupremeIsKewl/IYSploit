...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'YSploit is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
PluginsHint.Text = "Download plugins from the IYS WebSite (github.com)"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		...
		inviteButton.Text = "Copy Website Link (https://github.com)"
	end)
	dragGUI(main)
	main.Parent = PARENT
		...
		if pcall(function() readfile("IYS_FE.iys") end) then
			if readfile("IY_FE.iy") ~= nil then
				local success, response = pcall(function()
					...
					if json.prefix ~= nil then prefix = json.prefix else prefix = 'i' end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'exploit', DESC = 'Loads exploits GUI'}
...
CMDs[#CMDs + 1] = {NAME = 'hideiy', DESC = 'Hides the main IYS GUI'}
CMDs[#CMDs + 1] = {NAME = 'showiy', DESC = 'Shows IYS again'}
...
addcmd('exploit',{},function(args, speaker)



 notify('Incompatible Exploit', 'Your exploit does not support this command (missing guis)')
end)
...
minimizeHolder()
notify('IYS Is Ready', 'https://github.com/')
